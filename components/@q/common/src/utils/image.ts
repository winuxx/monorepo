...
export async function renderImgFile(file) {
...
export function transformSignatureImage(img, options?: ImgOptions) {
    const rate = options?.width / img.width || 1
...
export async function transformSignatureImageBySrc(src) {
    const img = await loadImage(src)
    return transformSignatureImage(img)
}

export async function transformSignatureImageByFile(file, options) {
    const img = await renderImgFile(file)
    return transformSignatureImage(img, options)
}
